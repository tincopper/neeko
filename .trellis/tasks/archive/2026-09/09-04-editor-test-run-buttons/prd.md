...
- R4 Rust debug 闭环（设计基准见 `design/test-run-debug.md`）：Debug 按钮 → 即开 DebugPanel（pending 态）→ 无头构建（后端 `debug_build_test_binary`，管道执行，不经 Task Console）→ 解析 compiler-artifact 拿唯一测试二进制 → 以 lldb launch 配置（program=测试二进制, args=[name]）启动 DAP 会话 → 既有 DebugPanel 交互（断点/栈帧/变量）不变。构建失败 / 0 产物 / 多产物 → 错误进 DebugPanel console tab + notification，Task Console 永不参与。
...
- [x] 用例检测纯函数：TS/JS test/it、Rust #[test]/#[tokio::test] 用例解析单测覆盖（含嵌套缩进、模板字符串名、多属性行）。
- [x] 测试文件内用例行上方出现 Run（+Rust 时 Debug）按钮；非测试文件/非 file tab 无按钮。
- [x] Run：vitest / cargo test 命令构造正确（纯函数断言）；点击后在 Task Console 出现会话并流式输出、可停止。
- [x] Debug（Rust）：点击即开 DebugPanel（pending 态）→ 无头构建 → 解析路径 → lldb 会话启动；构建失败 / 产物缺失 / 歧义时错误在 DebugPanel console 可见 + notification 且不启动会话；全程无 Task Console 会话。
- [x] 全程 TDD；`pnpm type-check` / `pnpm test:run` / `pnpm lint:fe` / `cargo test` 全绿。
...
- 设计唯一基准：`design/test-run-debug.md`（描述统一、载体分离；run 走 Task Console，debug 构建走无头后端命令）。