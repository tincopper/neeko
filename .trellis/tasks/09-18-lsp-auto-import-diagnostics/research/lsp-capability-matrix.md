...
| **jdtls：附加编辑是否内联返回** | ⚠️ **取决于我们的能力声明（2026-09-18 实证）** | jdtls 的 `extendedClientCapabilities.resolveAdditionalTextEditsSupport` 是"客户端会 resolve"的声明。实测（jdtls 1.61.0 + JDK 21，未导入 `java.util.List`）：声明 `true` → `List` 的 33 个候选 **0** 个带 `additionalTextEdits`（全部只在 `completionItem/resolve` 返回 `import java.awt.List;\n\n`）；不声明 → **33/33** 内联带编辑。本栈无 resolve 通道，故**禁止声明**（`plugin/builtins/java.rs` 已删并加单测钉死） |
