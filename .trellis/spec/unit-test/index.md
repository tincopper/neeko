...
1. **Red（写测试）**：先写测试定义预期行为，确认测试失败。
2. **Green（写实现）**：写最少代码让测试通过，不追求完美。
3. **Refactor（重构）**：在保持测试通过的前提下，消除重复、提升可读性。

### 新功能开发

```
定义接口 → 写测试 → 确认失败(Red) → 实现功能(Green) → 重构(Refactor) → 补充集成测试
```

### Bug 修复

```
写复现测试 → 确认失败(Red) → 修复代码(Green) → 确认通过 → 扩展测试覆盖
```
