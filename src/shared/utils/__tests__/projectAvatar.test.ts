...
describe('getAvatarStyle', () => {
  ...
  it('color 不在调色板内时走 hash 兜底（不接受任意 hex）', () => {
    ...
    expect((AVATAR_COLORS as readonly string[]).includes(result.color as string)).toBe(true);
  });
  ...
  it('不同的覆盖色互不影响 hash 兜底结果', () => {
    ...
    // hash 兜底应得到调色板内某色
    expect((AVATAR_COLORS as readonly string[]).includes(fallback.color as string)).toBe(true);
  });
});
