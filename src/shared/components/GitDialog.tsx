...
// eslint-disable-next-line import/no-restricted-paths -- GitDialog needs WSL home dir resolution; shared component depends on connection feature API
import { getWslHomeDir } from '../../features/connection/api/connectionApi';
// eslint-disable-next-line import/no-restricted-paths -- GitDialog needs git API for branch/worktree operations
...
const GitDialog: React.FC<GitDialogProps> = ({
  ...
  const handleCreateQuickWorktree = async () => {
    ...
    try {
      const src = dialog.source;
      if (src?.type === 'wsl') {
        await createWorktree(src.projectId, computedPath, name, true);
        onRefreshAfterWslSsh?.();
      } else if (src?.type === 'remote') {
  ...
  const handleCreateWorktree = async () => {
    ...
    try {
      const src = dialog.source;
      if (src?.type === 'wsl') {
        await createWorktree(src.projectId, worktreePath.trim(), worktreeBranch.trim(), newBranch);
        onRefreshAfterWslSsh?.();
      } else if (src?.type === 'remote') {
        setError('SSH worktree creation not yet supported');
        setSubmitting(false);
          ...
          <>
            ...
            {dialog.projectId && !dialog.source && (
              <div className="flex items-center gap-2 mb-3.5">
                <span
                  className={cn(
                    'text-xs text-text-muted font-medium uppercase tracking-[0.3px] transition-colors duration-150',
                    !customMode && 'text-text-primary',
                  )}
                >
                  Quick
                </span>
                ...
                >
                  <span
                    className={cn(
                      'absolute top-0.5 left-0.5 w-3 h-3 bg-text-secondary rounded-full transition-all duration-200',
                      customMode && 'translate-x-4 bg-accent-blue',
                    )}
                  />
                </button>
                <span
