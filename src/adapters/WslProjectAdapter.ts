...
export class WslProjectAdapter implements ProjectAdapter {
   ...
   getActiveProject(): UnifiedProject | null {
      ...
      const { distro, projectId } = this.activeWslKey;
      const entry = this.wslEntries.find((e) => e.distro === distro);
   ...
   async openIde(projectId: string, ide: string): Promise<void> {
      ...
      const project = entry.projects.find(p => p.id === projectId);
      if (!project) return;
      await invoke('open_wsl_ide', {
         distro: entry.distro, projectPath: project.path, ide
      });
   }
   ...
   async checkoutBranch(projectId: string, branchName: string): Promise<void> {
      ...
      const project = entry.projects.find(p => p.id === projectId);
      if (!project) return;
      await invoke('wsl_checkout_branch', {
         distro: entry.distro, projectPath: project.path, branchName
      });
   }
   ...
   async createBranch(projectId: string, branchName: string): Promise<void> {
      ...
      const project = entry.projects.find(p => p.id === projectId);
      if (!project) return;
      await invoke('wsl_create_branch', {
         distro: entry.distro, projectPath: project.path, branchName
      });
   }
   ...
   async createWorktree(projectId: string, worktreePath: string, branchName: string): Promise<void> {
      ...
      const project = entry.projects.find(p => p.id === projectId);
      if (!project) return;
      await invoke('wsl_create_worktree', {
         distro: entry.distro, projectPath: project.path, worktreePath, branchName, newBranch: false
      });
   }
