...
import { GitBranchPanel, GitWindow } from "./git";
...
function MainContent({
   ...
   // Git right-side panel width + drag resize
   const gitPanelOpen = gitViewState === "open";
   const [panelWidth, setPanelWidth] = useState(420);
   const dragRef = useRef<{ startX: number; origWidth: number } | null>(null);

   const handlePanelDragStart = useCallback((e: React.MouseEvent) => {
      e.preventDefault();
      dragRef.current = { startX: e.clientX, origWidth: panelWidth };
      document.body.style.userSelect = "none";
      document.body.style.cursor = "col-resize";

      const onMove = (ev: MouseEvent) => {
         if (!dragRef.current) return;
         const dx = dragRef.current.startX - ev.clientX; // drag left = wider
         setPanelWidth(Math.min(Math.max(250, dragRef.current.origWidth + dx), window.innerWidth * 0.8));
      };
      const onUp = () => {
   ...
   return (
      <div className="main-content flex-1 flex flex-row overflow-hidden bg-bg-primary">
         ...
         <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                  ...
                  {showAgentBarContent && (
                     <div className="h-8 px-2 pb-1 flex items-center gap-1">
                        <div className="relative shrink-0" ref={managerRef}>
                           ...
                           {managerOpen && (
                              <div className="absolute left-0 top-full mt-1 z-50 min-w-[180px] max-h-[280px] overflow-y-auto rounded-md border border-border bg-bg-secondary shadow-lg py-1">
            ...
            {/* Git minimized status bar */}
            {gitViewState === "minimized" && (
               <div
                  className="shrink-0 h-7 flex items-center px-2.5 gap-2 border-t border-border bg-bg-secondary cursor-pointer hover:bg-bg-hover transition-colors"
                  onClick={() => onRestoreGitView?.()}
               >
                  <GitBranch size={12} className="text-accent-green shrink-0" />
                  <span className="text-[11px] text-text-primary font-medium truncate">
                     {activeProject?.git_info?.current_branch ?? activeWslProject?.project.git_info?.current_branch ?? activeRemoteProject?.project.git_info?.current_branch ?? "HEAD"}
                  </span>
                  {activeProject && (
                     <span className="text-[10px] text-text-muted shrink-0">
                        {activeProject.git_info?.changed_files?.length ?? 0} changes
                     </span>
                  )}
               </div>
            )}
         ...
         {/* Right: Git side panel */}
         {gitPanelOpen && (
            <>
               {/* Drag handle */}
               <div
                  className="w-1 shrink-0 cursor-col-resize hover:bg-accent-blue/30 transition-colors"
                  onMouseDown={handlePanelDragStart}
               />
               {/* Panel */}
               <div
                  className="flex flex-col border-l border-border bg-bg-secondary overflow-hidden shrink-0"
                  style={{ width: panelWidth }}
               >
                  {/* Title bar */}
                  <div className="flex items-center justify-between px-3 py-1.5 bg-bg-tertiary border-b border-border shrink-0 select-none">
                     <span className="text-[calc(var(--font-size)-1px)] text-text-secondary font-medium">Git</span>
                     <div className="flex items-center gap-0.5">
                        <button
                           ...
                           onClick={() => onMinimizeGitView?.()}
                           title="Minimize"
                        >
                           <Minus size={12} />
                        </button>
                        <button
                           className="w-5 h-5 flex items-center justify-center rounded text-text-muted hover:text-text-primary hover:bg-bg-hover transition-colors text-xs"
                  ...
                  <div className="flex-1 min-h-0">
                     {activeProject && (
                        <GitWindow
                           gitSource={{ type: "local", projectId: activeProject.id }}
                           currentBranch={activeProject.git_info?.current_branch ?? "HEAD"}
                           diffMode={config.diffMode ?? "unified"}
                        />
                     )}
                     {activeWslProject && !activeProject && (
                        <GitBranchPanel
                           gitSource={{ type: "wsl", distro: activeWslProject.distro, projectPath: activeWslWorktreePath ?? activeWslProject.project.path }}
                           currentBranch={activeWslProject.project.git_info?.current_branch ?? "HEAD"}
                           diffMode={config.diffMode ?? "unified"}
                        />
                     )}
                     {activeRemoteProject && !activeProject && !activeWslProject && (
                        <GitBranchPanel
                           gitSource={{
                              ...
                              host: activeRemoteProject.entry.host,
                              port: activeRemoteProject.entry.port,
                              username: activeRemoteProject.entry.username,
                              auth: remoteAuthStore.get(activeRemoteProject.entry.id) ?? { Password: "" },
                              projectPath: activeRemoteWorktreePath ?? activeRemoteProject.project.path,
                           }}
                           currentBranch={activeRemoteProject.project.git_info?.current_branch ?? "HEAD"}
                           diffMode={config.diffMode ?? "unified"}
                        />
                     )}
                  </div>
