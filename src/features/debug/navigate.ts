...
import { fileRefFromTabPath, isJdtRef, sourceIdentityOf } from '@/shared/utils/fileRef';
...
async function openStopTab({
  ...
  if (existing) {
    ...
    store.setPendingNavigateTarget({ tabKey, tabId: existing.id, line: line1, col });
    return;
  }
  ...
  fresh.setPendingNavigateTarget({ tabKey, tabId, line: line1, col });
}
...
export async function openSourceAtLine(
  ...
  //   FileRef；JDK 解压缓存路径也在此收敛成 jdt 身份，使断点 key / 黄线天然一致）；
  // - `loadPath` 是**实际可读路径**（缓存文件就是磁盘上的真文件）。
  ...
  // 虚拟文档已打开时 openStopTab 走「复用既有 tab」分支直接激活；未打开则无法
  // 重建（jdtls 的 classFileContents 需要原始 `jdt://…?<query>`，展示路径不含
  // query），给出明确提示而不是去读一个不存在的文件。
  const load = isJdtRef(ref)
    ? async (): Promise<StopSourceContent> => ({
        kind: 'failed',
        error: new Error('virtual document is not open — jump to the class from the editor again'),
      })
    : () => loadStopSourceContent(projectId, loadPath, opts?.sessionId);
