 ...
 * （Go 的 `t.Run` 子测试，见 `languages/go/overlays.ts`）。本模块是共享的产物解析层，
...
/**
 * 对齐后的单条结果 —— 与 `store/testResults::AlignedCaseResult` **结构一致**。
 *
 * 本模块刻意零依赖（纯解析），故不复用 store 的类型；调用方按结构直接落库。
 */
...
/**
 * 事件流 → 对齐到源码用例名的结果（libtest / test2json 共用）。
 *
 * 放在格式层而非语言模块：对齐规则由**该格式的**命名形态决定（`matchCaseName`），
 * 两条链路各自实现会漂移。
 */
export function alignEvents(events: LibtestEvent[], caseName: string): AlignedRow[] {
  const results: AlignedRow[] = [];
  for (const event of events) {
    if (!matchCaseName(event.name, caseName)) continue;
    results.push({
      caseName,
      status: event.status,
...
/** 报告行（vitest / JUnit XML）→ 对齐到源码用例名的结果（字段名由 `nameOf` 取）。 */
export function alignReportRows<T extends { status: string; duration?: number; message?: string }>(
  rows: readonly T[],
  nameOf: (row: T) => string,
  caseName: string,
): AlignedRow[] {
  return rows
    .filter((row) => matchCaseName(nameOf(row), caseName))
    .map((row) => ({
      caseName,
      status: row.status as AlignedRow['status'],
