 ...
 */
const SYNTAX_DIR = 'src/features/runner/syntax';
const LANGUAGES_DIR = 'src/features/runner/languages';

/** 发现路径的全部模块（语言实现 + 共享工具 + 契约）。 */
const DISCOVERY_MODULES = [
  `${LANGUAGES_DIR}/go/discover.ts`,
  `${LANGUAGES_DIR}/go/table.ts`,
  `${LANGUAGES_DIR}/rust/discover.ts`,
  `${LANGUAGES_DIR}/java/discover.ts`,
  `${LANGUAGES_DIR}/ts/discover.ts`,
  `${SYNTAX_DIR}/lezer.ts`,
  `${SYNTAX_DIR}/parsers.ts`,
  `${SYNTAX_DIR}/contract.ts`,
] as const;
...
describe('P4-4 收口护栏：发现路径单一机制（AST）', () => {
  it('languageSyntax.ts 已退役（声明形态不再有第二处落点）', () => {
    expect(existsSync('src/features/runner/utils/languageSyntax.ts')).toBe(false);
  });

  it('发现模块不含「用正则匹配源码」的实现（`new RegExp` / `.exec(` / `.match(`）', () => {
    // 说明：`ts.ts` 里的 `String.replace(/…/)` 是**值反转义**（历史行为保持），不是结构匹配，
    // 故不在禁止之列 —— 本护栏针对的是「靠正则定位声明」这一机制。
    const offenders = DISCOVERY_MODULES.map((path) => ({
      path,
      hits: /new RegExp|\.exec\(|\.match\(/.test(read(path)),
  ...
  it('语言模块不提供逐行正则（文本）发现入口', () => {
    // 字段本身已从语言模块接口删除（`LanguageModule.discover` 只能吃语法树）；此处再从源码层
    // 确认没有任何语言模块把「逐行正则」机制加回来。
    const offenders = [
      `${LANGUAGES_DIR}/registry.ts`,
      ...LANGS.map((lang) => `${LANGUAGES_DIR}/${lang}/index.ts`),
    ]
      ...
      .map((path) => ({
        ...
        hits: read(path)
          .split('\n')
          .filter((line) => /^\s*(parseTestCases|parseMainEntries)\s*[:(]/.test(line)).length,
      }))
      .filter((entry) => entry.hits > 0);
    expect(offenders).toEqual([]);
  });

  it('四个语言都提供 AST 发现入口（`discover` + `discover.ts`）', () => {
    const missing = LANGS.filter((lang) => {
      const index = `${LANGUAGES_DIR}/${lang}/index.ts`;
      const discover = `${LANGUAGES_DIR}/${lang}/discover.ts`;
      return !existsSync(discover) || !/discover:/.test(read(index));
    });
