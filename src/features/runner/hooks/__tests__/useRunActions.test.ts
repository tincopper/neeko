...
const mockStartJavaDebug = vi.hoisted(() => vi.fn());
const mockSetJavaBackendLabel = vi.hoisted(() => vi.fn());
...
vi.mock('@/features/runner/store/debugStore', () => ({
  useDebugStore: {
    getState: () => ({
      ...
      refreshStackAndVars: vi.fn(),
    }),
  },
}));

// Java 会话状态与动作已迁到**语言自己的 store**（方案 B 阶段 4）：mock 目标随之改指。
vi.mock('@/features/runner/store/javaDebugStore', () => ({
  useJavaDebugStore: {
    getState: () => ({
      startJavaAttach: mockStartJavaAttach,
      startJavaDebug: mockStartJavaDebug,
      setBackendLabel: mockSetJavaBackendLabel,
      markHostFallback: vi.fn(),
      clearHostFallback: vi.fn(),
      isHostFallback: () => false,
      resetSession: vi.fn(),
      markZeroTestReported: vi.fn(),
      backendLabel: null,
...
// 本文件聚焦 **A（自写 host）路径** 的回归：显式把后端钉成 `host`。
// B'（JDTLS 后端）的 dispatch 决策由 `shared/utils/__tests__/javaDebugBackend.test.ts`
// 覆盖，端到端由真机 spike 验证（见 research/jdtls-debug-spike.md）。
vi.mock('@/features/settings/api/settingsApi', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/features/settings/api/settingsApi')>();
  return { ...actual, loadJavaDebugBackend: async () => 'host' };
});
...
import { clearCargoManifestCache } from '@/features/runner/languages/rust/manifest';
import {
  statusForCase,
  subtestsForCase,
  useTestResultsStore,
  ...
  describe('run result stream (libtest JSON / vitest report → testResults store)', () => {
    ...
    it('Go benchmark：包级终态收口 → 落 ✓ 且不触发「零命中」告警（P2 × P0 联动）', async () => {
      ...
      act(() =>
        result.current.handleRun({
          kind: 'test',
          testCase: { name: 'BenchmarkAdd', line: 4, lang: 'go', variant: 'benchmark' },
        }),
      );
      await waitFor(() => expect(mockStart).toHaveBeenCalledTimes(1));
        ...
        result.current.openMenu(
          {
            kind: 'test',
            testCase: { name: 'TestTable', line: 3, lang: 'go' },
            overlay: { subtests: ['TestTable/static'] },
          },
          10,
          20,
        ),
        ...
        result.current.openMenu(
          {
            kind: 'test',
            testCase: { name: 'TestTable', line: 3, lang: 'go' },
            overlay: { subtests: ['TestTable/zero'] },
          },
          10,
          20,
        ),
