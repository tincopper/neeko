...
function isRange(v: unknown): v is { start: { line: number; character: number } } {
  ...
  return !!start && typeof start.line === 'number' && typeof start.character === 'number';
}

function walkDocumentSymbol(node: Record<string, unknown>, depth: number, out: FlatSymbol[]): void {
