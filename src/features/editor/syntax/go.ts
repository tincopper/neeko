 ...
 * - `Test*` → 用例；`Benchmark*` → 基准（`kind: 'benchmark'`）；其余（`Example*` / `Fuzz*` /
...
import { collectTableSubtests } from './goTable';
import { childOfType, createLineLookup, walkPruned, type SyntaxNode } from './lezer';
...
export function discoverGoTests(sd: SyntaxDoc): TestCaseInfo[] {
  ...
  walkPruned(tree.topNode, cutFunctionBodies, (node) => {
    ...
    if (name.startsWith('Test')) {
      ...
      cases.push({ name, line, lang: 'go', kind: 'benchmark' });
    }
  });
