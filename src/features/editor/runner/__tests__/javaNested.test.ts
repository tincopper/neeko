...
vi.mock('@/features/debug/api/debugBuildApi', () => ({ buildTestBinaryRemote: vi.fn() }));
vi.mock('@/features/debug/store/debugStore', () => ({ useDebugStore: { getState: () => ({}) } }));
...
import { withJavaNestedClassPath } from '../java';
...
describe('withJavaNestedClassPath（@Nested 内层类富化）', () => {
  it('命中嵌套方法 → 附加 nestedClassPath；uri 走 canonical 助手', async () => {
    mockLspRequest.mockResolvedValue(flatSymbols());
    const testCase = { name: 'testNested', line: 12, lang: 'java' as const };

    const enriched = await withJavaNestedClassPath(ctx, testCase);

    expect(enriched).toEqual({ ...testCase, nestedClassPath: ['InnerCases'] });
  ...
  it('顶层方法 → 不附加字段（与历史形态一致）', async () => {
    ...
    expect(await withJavaNestedClassPath(ctx, testCase)).toEqual(testCase);
  });
  ...
  it('无项目根（projectPath 为 null）→ 不发请求、原样返回', async () => {
    ...
    expect(await withJavaNestedClassPath({ ...ctx, projectPath: null }, testCase)).toEqual(
      testCase,
    );
    expect(mockLspRequest).not.toHaveBeenCalled();
  });
