...
describe('分层护栏：gutter 四模块依赖单向', () => {
  it('L0 叶子（runTarget / runLspOverlay）不依赖任何上层', () => {
    const offenders = (['runTarget.ts', 'runLspOverlay.ts'] as const).filter((name) =>
      importsUp(read(name), [MARKERS, CONTRIBUTION]),
    );
  ...
  it('L2 装配层依赖下层（正向），且公开 API 仍在装配层导出', () => {
    ...
    expect(importsUp(contribution, [MARKERS, 'runTarget', 'runLspOverlay'])).toBe(true);
    for (const api of [
  ...
  it('目标身份（RunTarget 等）由 L0 runTarget 持有，装配层不再重复定义', () => {
    const target = read('runTarget.ts');
