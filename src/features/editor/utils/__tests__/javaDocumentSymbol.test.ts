...
import { nestedClassPath, parseJavaSymbols } from '../javaDocumentSymbol';
import { buildRunCommand, resolveRunContext } from '../runLanguages';
import { junitLauncherJarName, type JavaRunEnv } from '../testCommands';
...
describe('nestedClassPath（方法 → 内层类链，不含最外层类）', () => {
  const symbols = parseJavaSymbols(flatPayload());

  it('顶层方法 → 空链（最外层类由 deriveJavaFqcn 负责，不重复携带）', () => {
    expect(nestedClassPath(symbols, 'top', 7)).toEqual([]);
  });

  it('一层 @Nested → ["L1"]', () => {
    expect(nestedClassPath(symbols, 'mid', 12)).toEqual(['L1']);
  });

  it('两层 @Nested → ["L1","L2"]（外→内）', () => {
    expect(nestedClassPath(symbols, 'deep', 17)).toEqual(['L1', 'L2']);
  });

  it('未知方法 → 空链（降级为现状表单）', () => {
    expect(nestedClassPath(symbols, 'nope', 1)).toEqual([]);
  });
  ...
  it('同名方法：取「名字行 ≥ 注解行」的最近者，不串味', () => {
    ...
    expect(nestedClassPath(dup, 'dup', 9)).toEqual(['A']);
    expect(nestedClassPath(dup, 'dup', 19)).toEqual(['A', 'B']);
  });
});
...
describe('e2e：真实 jdt.ls 载荷 → @Nested 选择器', () => {
  ...
  it('testNested：链 = ["InnerCases"]，完整命令含真机可执行的 $ 选择器', async () => {
    ...
    expect(nestedClassPath(symbols, 'testNested', 12)).toEqual(['InnerCases']);
    ...
    const ctx = await resolveRunContext('java', relPath, '/proj', { javaEnv: env });
    const command = buildRunCommand(testCase, relPath, null, '/proj', ctx);
  ...
  it('testTop：顶层方法 → 空链，选择器无 $（与历史形态一致）', () => {
    const symbols = parseJavaSymbols(realPayload);
    expect(nestedClassPath(symbols, 'testTop', 7)).toEqual([]);
  });
});
