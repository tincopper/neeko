 ...
 * 断点红点常驻；测试 play 标记 + 用例状态回显仅可编辑的测试文件叠加，
 * 非 file tab / 无上下文返回空数组（零列，零开销）。
 *
 * 装配点职责（registry 架构的值层）：把各域贡献拼进注册表——
 * debug 的 breakpointContribution（经 `@/features/debug` 门面，只读消费其
 * 自备扩展；toggle/hover 行为经回调注入）+ editor 自家的 test-run / test-status
...
import { createTestCodelensCore, createTestRunContribution } from '../gutter/testRunContribution';
import {
...
interface UseUnifiedGutterExtensionParams {
  ...
  fileName: string;
  /** file tab 可编辑性门控（readOnly / binary / 超大文件 → 仅断点列，无测试标记）。 */
  enabled: boolean;
  /** TS/JS 用例点击直接运行（扩展内按 lang 分流，Rust 用例走 onMenuRequest）。 */
  onRun: (testCase: TestCaseInfo) => void;
  /** Rust 用例点击 → 请求打开 Run/Debug 浮层（x/y 为图标 rect 旁锚点，非鼠标裸坐标）。 */
...
export function useUnifiedGutterExtension({
  ...
  const isTestOrRust = isTestFile(fileName) || fileName.endsWith('.rs');

  return useMemo(() => {
    if (!projectId || !absFilePath) return [];
    const withTests = enabled && isTestOrRust;
    const testRun = withTests ? createTestRunContribution({ onRun, onMenuRequest }) : null;
    const testStatus = withTests
    ...
    const assembled: Extension[] = [
      ...
      ...(testRun
        ? [
            createTestCodelensCore({ fileName, onRun, onMenuRequest }),
