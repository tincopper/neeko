 ...
 *
 * 命令构造不放这里（`runnableToCommand` 之类在 `utils/testCommands.ts`，与其它命令构造器
 ...
 */
export function parseRunnables(raw: unknown): LspRunnable[] {
  if (!Array.isArray(raw)) return [];
  const out: LspRunnable[] = [];
...
export function isSpecificTestRun(runnable: LspRunnable): boolean {
...
export function selectRunnable(
  runnables: readonly LspRunnable[],
  target: RunnableTarget,
): LspRunnable | null {
  const required = REQUIRED_CARGO_SUBCOMMAND[target];
  const scored: { runnable: LspRunnable; tier: number; tokens: number; index: number }[] = [];
