...
import { isSpecificTestRun, parseRunnables, selectRunnable, type LspRunnable } from '../runnable';
...
describe('selectRunnable', () => {
  const parsed = (raw: unknown[]): LspRunnable[] => parseRunnables(raw);
  ...
  it('测试：在同一位置的多种粒度中选「具体用例」而非 check / test --all-targets', () => {
    ...
    expect(isSpecificTestRun(chosen as LspRunnable)).toBe(true);
  });
