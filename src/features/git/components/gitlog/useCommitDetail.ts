...
export function useCommitDetail(
  ...
  // Derive display values: null/[] when commitHash is null (avoids sync setState in effect)
  return { detail: commitHash ? detail : null, files: commitHash ? files : [], loading, error };
}
