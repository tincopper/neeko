...
describe('documentOwnership — 编辑器文档所有权的引用计数', () => {
  ...
  it('单视图：挂载声明一次、卸载释放一次', () => {
    ...
    expect(lspApi.lspClaimDocument).toHaveBeenCalledTimes(1);

    releaseDocumentOwnership('/p', 'rust', 'file:///a.rs');
    expect(lspApi.lspReleaseDocument).toHaveBeenCalledTimes(1);
  });
  ...
  it('多视图：0→1 才声明、1→0 才释放', () => {
    ...
    releaseDocumentOwnership('/p', 'rust', 'file:///a.rs');
    expect(lspApi.lspReleaseDocument).not.toHaveBeenCalled();
  ...
  it('不同（project, language, uri）各自独立计数', () => {
    ...
    expect(lspApi.lspClaimDocument).toHaveBeenCalledTimes(3);
