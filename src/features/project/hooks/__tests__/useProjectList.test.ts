...
describe('useProjectList', () => {
  it('returns empty for no projects', () => {
    const { result } = renderHook(() => useProjectListFromData([]));
    expect(result.current.items).toHaveLength(0);
    expect(result.current.isEmpty).toBe(true);
  });

  it('returns local projects in order', () => {
    const { result } = renderHook(() => useProjectListFromData([proj('p1'), proj('p2')]));
  ...
  it('sets has_git_info based on git_info field', () => {
    const { result } = renderHook(() =>
      useProjectListFromData([proj('p1'), proj('p2', { git_info: {} })]),
    );
  ...
  it('sets selected_agents from project', () => {
    const { result } = renderHook(() =>
      useProjectListFromData([proj('p1', { selected_agents: ['agent-1'] })]),
    );
