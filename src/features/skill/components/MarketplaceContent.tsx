...
interface MarketplaceContentProps {
  ...
  /** Hide the built-in "Marketplace" header row (e.g. when a breadcrumb already provides it). */
  titleless?: boolean;
}

const MarketplaceContent: React.FC<MarketplaceContentProps> = React.memo(
  ({ onSkillInstalled, titleless }) => {
    const skills = useSkillStore((s) => s.skills);
    const refreshSkills = useSkillStore((s) => s.refreshSkills);
    const deleteSkill = useSkillStore((s) => s.deleteSkill);
    const setMarketplaceTotalItems = useSkillStore((s) => s.setMarketplaceTotalItems);
    ...
    const handleUninstall = useCallback(
      async (marketSkillId: string) => {
        const managedId =
          ...
          skills.find(
            (s) => s.name === marketSkillId || s.name.toLowerCase() === marketSkillId.toLowerCase(),
          )?.id;
        if (!managedId) {
    ...
    useEffect(() => {
      setMarketplaceTotalItems(totalItems);
    }, [totalItems, setMarketplaceTotalItems]);
