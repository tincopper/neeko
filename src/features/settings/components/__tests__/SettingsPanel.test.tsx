...
describe('SettingsPanel', () => {
  ...
  describe('Editor 导航', () => {
    ...
    it('中间值可以正常增减', () => {
      ...
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ editorFontSize: 15 }));
    });
    ...
    it('字号被 clamp 在 10-24 范围', () => {
      ...
      fireEvent.click(screen.getByRole('button', { name: 'Editor' }));
      fireEvent.click(screen.getByText('\u2212'));
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ editorFontSize: 10 }));
    });
  ...
  describe('Shell 预设', () => {
    ...
    it('点击预设按钮更新 shell', async () => {
      ...
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ shell: '/bin/bash' }));
    });
  });
  ...
  describe('自定义 IDE', () => {
    ...
    it('已有自定义 IDE 时可以删除', () => {
      ...
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ customIdes: [] }));
    });
  });
  ...
  describe('自定义 Agent', () => {
    ...
    it('已有自定义 Agent 时可以删除', () => {
      ...
      const removeBtn = screen.getByTitle('Remove');
      fireEvent.click(removeBtn);
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ customAgents: [] }));
    });
  });
  ...
  describe('GitPanel ToggleGroup', () => {
    ...
    it('点击 Split 切换 diff 模式', () => {
      ...
      expect(onConfigChange).toHaveBeenCalledWith(expect.objectContaining({ diffMode: 'split' }));
    });
  });
});
