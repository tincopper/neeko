    ...
    if (existingCache) {
      ...
      void (async () => {
        ...
        try {
          ...
          // 克隆+stringify 完成值导致 WebContent RSS 只增不减；invoke 走
          // custom protocol fetch，零 eval 零克隆。默认 long-poll 挂起式
          // drain（createDrainTransportScheduler），VITE_TERMINAL_DRAIN_POLL=1
          // 时回退 100ms 全局轮询。
