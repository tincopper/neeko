...
export function useRemoteAuthActions({
  saveSession,
}: UseRemoteAuthActionsParams): UseRemoteAuthActionsResult {
  ...
  const handleRemoteAuthSuccess = useCallback(
    (auth: AuthMethod, saved_auth?: string | null) => {
      ...
      const updatedEntries = snapshot.remoteEntries.map((entry) =>
        entry.id === entryId ? { ...entry, saved_auth } : entry,
      );
