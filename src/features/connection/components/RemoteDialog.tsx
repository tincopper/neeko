...
export function RemoteDialog({
  ...
  const handleConnect = async () => {
    ...
    setConnecting(true);
    try {
      await testRemoteConnection(host, parseInt(port) || 22, username, auth);
      setStep('add-project');
      // 连接成功后聚焦路径输入并触发初始补全
      setTimeout(() => {
        pathInputRef.current?.focus();
  ...
  const previewName =
    projectPath.replace(/\/$/, '').split('/').filter(Boolean).pop() || projectPath;
  ...
  return (
    ...
    >
      <DialogContent className="min-w-[460px] max-w-[560px]">
        <DialogHeader>
          <DialogTitle>
            {step === 'server-config' ? 'Add Remote Server' : 'Add Remote Project'}
          </DialogTitle>
        </DialogHeader>
