  ...
  const handleConfirmPath = () => {
    ...
    if (selectedIdeId) {
      if (selectedIdeId.startsWith('custom:')) {
        ...
        if (preset)
          selectedIdeCommand = config.ideCommandOverrides?.[preset.id] ?? getIdeCommand(preset);
      }
    }
  ...
  return (
    ...
    >
      <DialogContent className="max-w-[560px]">
        <DialogHeader>
          <DialogTitle>
            {step === 'select-distro' ? 'Select WSL Distro' : 'Add WSL Project'}
          </DialogTitle>
        </DialogHeader>
        ...
        {step === 'select-distro' ? (
          <>
            {loadingDistros ? (
              ...
              <p className="text-text-secondary text-center py-5">
                No WSL distros found. Please install a WSL distro first.
              </p>
            ) : (
          ...
          <>
            <div className="flex items-center gap-2.5 px-3 py-2.5 bg-bg-tertiary rounded-md mb-4">
              <img
                className="text-[18px] text-text-secondary"
                src={getDistroIcon(selectedDistro)}
                width={15}
                height={15}
                alt=""
              />
                    ...
                    suggestions.map((s, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="font-mono text-[11px] text-text-muted whitespace-nowrap shrink-0 ml-1">
                            {parent}
                          </span>
                        </div>
                      );
            ...
            {inputPath && inputPath !== '/' && (
              <div className="flex items-center gap-2.5 px-3 py-2.5 bg-[rgba(97,175,239,0.08)] border border-[rgba(97,175,239,0.3)] rounded-md mt-3">
                ...
                <span className="font-mono text-[13px] text-accent-blue font-medium whitespace-nowrap overflow-hidden text-ellipsis">
                  {previewName}
                </span>
              </div>
            )}
            ...
            <div className="relative w-full mt-1" ref={ideDropdownRef}>
              ...
              >
                {(() => {
                  const selectedPreset =
                    selectedIdeId && !selectedIdeId.startsWith('custom:')
                      ? IDE_PRESETS.find((i) => i.id === selectedIdeId)
                      : null;
                  if (selectedPreset) {
                    return (
                      <>
                        <img
                          src={getIdeIconSrc(selectedPreset.icon)}
                          alt=""
                  ...
                  return (
                    <>
                      <span className="font-medium opacity-50">None (VSCode/Cursor/Zed)</span>
                    </>
                  );
                })()}
                <span className="text-xs text-text-secondary ml-auto">
              ...
              {ideDropdownOpen && (
                <div className="absolute top-full mt-1 bg-bg-secondary border border-border rounded-md shadow-lg z-[100] overflow-hidden left-0 right-0 min-w-[unset]">
                  ...
                  {IDE_PRESETS.filter((p) => ['vscode', 'cursor', 'zed'].includes(p.id)).map(
                    (preset) => (
                      <div
                        key={preset.id}
                        role="option"
