 ...
 * 固定底部面板注册表（单一事实源）。
 *
 * 新增固定面板只改这里 —— 组合根（App.tsx）经 FixedPanelsHost 按 registry
 * 渲染，对面板清单零感知（与 dockPanelRegistry 同一模式，OCP：面板扩展
 * 不触碰组合根）。lazy 化：面板代码不进主 chunk，首次渲染按需加载。
 */
export const fixedPanelRegistry: FixedPanelDef[] = [
  { id: 'task-console', Component: LazyTaskConsolePanel },
  { id: 'debug', Component: LazyDebugPanel },
];
