...
pub async fn fetch_leaderboard(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let dtos: Vec<SkillsShSkillDto> = skills.into_iter().map(|s| SkillsShSkillDto {
            id: s.id,
            skill_id: s.skill_id,
            name: s.name,
            source: s.source,
...
pub async fn search_skillssh(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let proxy_url = store.get_setting("proxy_url").ok().flatten();
        let skills = super::skillssh_api::search_skills(&query, limit.unwrap_or(20), proxy_url.as_deref())
            .map_err(|e| e.to_string())?;
...
pub async fn install_from_skillssh(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let proxy_url = store.get_setting("proxy_url").ok().flatten();
        let repo_path = super::git_fetcher::clone_repo_ref(&git_url, None, None, proxy_url.as_deref())
            .map_err(|e| e.to_string())?;
