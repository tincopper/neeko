...
pub fn build_http_client(proxy_url: Option<&str>, timeout_secs: u64) -> Result<reqwest::blocking::Client> {
...
pub fn search_skills(query: &str, limit: usize, proxy_url: Option<&str>) -> Result<Vec<SkillsShSkill>> {
    ...
    let url = format!("https://skills.sh/search?q={}&limit={}", encoded_query, limit);
...
fn parse_next_data(html: &str) -> Result<Vec<SkillsShSkill>> {
    ...
    if let Some(items) = data.pointer("/props/pageProps/skills").and_then(|v| v.as_array()) {
        for item in items {
            if let Some(skill) = parse_skill_from_json(item) {
                skills.push(skill);
            }
        }
    } else if let Some(items) = data.pointer("/props/pageProps/results").and_then(|v| v.as_array()) {
        for item in items {
            if let Some(skill) = parse_skill_from_json(item) {
                skills.push(skill);
            }
...
fn parse_skill_from_json(item: &serde_json::Value) -> Option<SkillsShSkill> {
    ...
    let skill_id = item.get("id").or_else(|| item.get("skill_id"))?.as_str()?.to_string();
    let name = item.get("name").or_else(|| item.get("title"))?.as_str()?.to_string();
    let installs = item.get("installs").or_else(|| item.get("downloads"))?.as_u64().unwrap_or(0);
...
fn parse_rsc_payload(html: &str) -> Result<Vec<SkillsShSkill>> {
    ...
    let re_block = Regex::new(r#"\{"source":"[^"]+","(?:skill_)?id":"[^"]+","(?:name|title)":"[^"]+"[^}]*\}"#)?;
    
    for cap in re_block.captures_iter(html) {
        let block = cap.get(0).unwrap().as_str();
        
        let source = re_source.captures(block)
            .and_then(|c| c.get(1))
            .map(|m| m.as_str().to_string())
            .unwrap_or_default();
        
        let skill_id = re_id.captures(block)
            .and_then(|c| c.get(1))
            .map(|m| m.as_str().to_string())
            .unwrap_or_default();
        ...
        let installs = re_installs.captures(block)
            .and_then(|c| c.get(1))
