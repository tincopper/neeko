...
pub async fn update_mcp_tag_group_cmd(
    ...
    run_blocking_result(move || {
        ...
        store.update_mcp_tag_group(&group).map_err(AppError::from)?;
        let server_ids = store
...
async fn test_mcp_connection_logic(
    ...
    let outcome = if server.transport == "http" {
        ...
        crate::library::skill::mcp_probe::McpProbe::probe_http(url).await
    } else if server.transport == "sse" {
        let url = server.url.as_deref().unwrap_or("");
        crate::library::skill::mcp_probe::McpProbe::probe_sse(url).await
    } else {
        crate::library::skill::mcp_probe::McpProbe::probe_stdio(&server.command, &args, &env).await
    };
...
pub struct McpRegistrySearchDto {
    ...
    pub servers: Vec<crate::library::skill::mcp_registry_api::McpRegistryServerSummary>,
...
pub async fn search_mcp_registry_cmd(
    ...
    let (servers, next_cursor) = run_blocking_result(move || {
        ...
        crate::library::skill::mcp_registry_api::search_registry(
            &query,
            limit,
            cursor.as_deref(),
            proxy_url.as_deref(),
    ...
    let servers = run_blocking_result(move || {
        ...
        let client = crate::library::skill::mcp_registry_api::build_http_client(proxy_url.as_deref())
            .map_err(AppError::from)?;
            ...
            std::thread::scope(|scope| {
                for s in chunk {
                    scope.spawn(|| {
                        ...
                        let (stars, downloads) =
                            crate::library::skill::mcp_registry_api::fetch_server_metrics(s, &client);
                        s.stars = stars;
...
pub async fn fetch_mcp_registry_server_cmd(
    name: String,
    store: State<'_, Arc<McpStore>>,
) -> Result<crate::library::skill::mcp_registry_api::McpRegistryServerDetail, AppError> {
    let store = store.inner().clone();
    ...
    if let Ok(Some(cached)) = store.get_cache(&cache_key, 300) {
        if let Ok(detail) =
            serde_json::from_str::<crate::library::skill::mcp_registry_api::McpRegistryServerDetail>(&cached)
        {
            return Ok(detail);
        }
    }
    ...
    let detail = run_blocking_result(move || {
        ...
        crate::library::skill::mcp_registry_api::fetch_server(&name, proxy_url.as_deref())
            .map_err(AppError::from)
    })
    .await?;
