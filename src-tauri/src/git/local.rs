    ...
    for path in file_paths {
        ...
        if !output.status.success() {
            ...
            if stderr.contains("unknown revision") || stderr.contains("ambiguous") {
                let rm_output = no_window_cmd("git")
...
pub fn discard_file(repo_path: &Path, file_path: &str) -> Result<()> {
    ...
    if x != b' ' && x != b'?' {
        ...
        if !reset.status.success() {
            ...
            if x == b'A' {
                let rm_cached = no_window_cmd("git")
    ...
    if y != b' ' && y != b'?' {
        let checkout = no_window_cmd("git")
