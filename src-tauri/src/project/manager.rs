//! `ProjectManager`：项目列表、生命周期与持久化回调的管理器。
//!
//! 抽自 `mod.rs`（Slim mod.rs 红线：业务实现不留在模块入口文件），
//! 经 `mod.rs` 的 `pub use manager::*` 在 `crate::project` 下 re-export，
//! 对外路径 `crate::project::ProjectManager` 保持不变，引用方零改动。
...
impl ProjectManager {
    ...
    pub fn add_project_from_session(&mut self, session: &ProjectSession) -> Result<Project> {
        ...
        // 恢复时重建 git_info：持久化类型 ProjectSession 不含 git_info（易变状态不入盘），
        // 若留空则前端 bootstrap 会把 null 误判为「非 git 项目」，跳过 changed_files 与
        // ignored_files 拉取 —— 文件树 git 状态色与忽略灰化将失效（直到首个 git 事件到来）。
        // 语义与 add_project 对齐：Local 项目 git_info 非 null ⟺ 是 git 仓库；
        // WSL/Remote 不做本地路径检测（其 git 数据由激活后的 watcher/命令填充）。
        let git_info = if matches!(session.environment, ProjectEnvironment::Local)
            && git::is_git_repo(&session.path)
        {
            git::get_git_info(&session.path).ok()
        } else {
            None
        };
        let project = Project {
