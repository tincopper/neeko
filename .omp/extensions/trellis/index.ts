...
// single turn (input, before_agent_start, and context may all fire closely)
// ---------------------------------------------------------------------------
...
export default function(pi: ExtensionAPI): void {
   ...
   // and post-compaction agent.continue() paths). This is the reliable injection
   // point — before_agent_start only fires once/turn and may miss continuations.
   pi.on("context", async (event) => {
      if (!projectRoot) return;
      const cached = turnCache.get(projectRoot);
      if (!cached.workflowMsg) return;

      // Skip if already present (e.g. before_agent_start persisted it earlier)
      const alreadyPresent = event.messages.some(
         (msg: { role?: string; customType?: string }) =>
            msg.role === "custom" && msg.customType === "trellis-workflow-state",
      );
   ...
   pi.on("input", async (_event, ctx) => {
      ...
      // Pre-warm the cache so that before_agent_start and context can use it
